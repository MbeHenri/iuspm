...
class BaseService {
    ...
    async getStudent(uuid: string): Promise<Student> {
        return await this.base_rep.getStudent(uuid)
    }
    ...
    async getStudentNotes(student: Student): Promise<Map<Semester, NoteSimple[]>> {
        ...
        const notes: any[] = await this.base_rep.getStudentNotes(student.uuid)
    ...
    async updateStudentNote(note: NoteSimple, student: Student): Promise<NoteSimple> {
        const data = (note.uuid ?
            await this.base_rep.updateStudentNote(note.uuid, note.cc ?? 0, note.ef ?? 0, note.isNormal ?? true, note.ec.uuid, student.uuid) :
            await this.base_rep.createStudentNote(note.cc ?? 0, note.ef ?? 0, note.isNormal ?? true, note.ec.uuid, student.uuid))
